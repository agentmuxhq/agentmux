    ...
    engine.register_handler(
        ...
        Box::new(|data, _ctx| {
            Box::pin(async move {
                ...
                let working_dir_expanded = if cmd.working_dir.starts_with("~/") || cmd.working_dir == "~" {
                    if let Some(home) = std::env::var_os("HOME")
                        .or_else(|| std::env::var_os("USERPROFILE"))
                    {
                        cmd.working_dir.replacen("~", &home.to_string_lossy(), 1)
                    } else {
                ...
                let base_path = std::path::Path::new(&working_dir_expanded);
                if !base_path.exists() {
...
/// Run `<cli> --version` and return the version string, or "unknown".
/// Check if an install failure looks like a network error; return appropriate message.
