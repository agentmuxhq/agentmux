# Build agentmuxsrv for Windows x64
...
Write-Host "Building agentmuxsrv.x64.exe with Zig..."
...
& $goExe build `
    ...
    -o dist/bin/agentmuxsrv.x64.exe `
    cmd/server/main-server.go

if ($LASTEXITCODE -eq 0) {
    Write-Host "Build successful!" -ForegroundColor Green
    Get-Item dist/bin/agentmuxsrv.x64.exe | Select-Object Name, Length, LastWriteTime
} else {
