...
The `file` command provides a set of subcommands for managing files across different storage systems, such as `muxfile`, `wsh` remote servers, and S3.
...
- `muxfile` - Used to retrieve blockfiles from the internal AgentMux filesystem.

  Format: `muxfile://[zoneid]/[path]`

  AgentMux file locations can be:

  - `muxfile://block/...` - stored in the current block ("this" is also an alias for "block")
  - `muxfile://tab/...` - stored in the current tab
  - `muxfile://workspace/...` - stored in the current workspace ("ws" is also an alias for "workspace")
  - `muxfile://client/...` - stored globally for the client ("global" is also an alias for "client")
  - `muxfile://temp/...` - stored globally, but removed on startup/shutdown
  - `muxfile://[uuid]/...` - an entity id (can be a block, tab, workspace, etc.)
...
```sh
wsh file cat muxfile://block/config.txt
...
```sh
echo "hello" | wsh file write muxfile://block/greeting.txt
...
```sh
tail -f app.log | wsh file append muxfile://block/logs.txt
echo "new line" | wsh file append muxfile://client/notes.txt
```
...
wsh file cp muxfile://block/config.txt ./local-config.txt
...
wsh file mv muxfile://block/config.txt ./local-config.txt
...
wsh file ls muxfile://client/ | grep ".json$"
```
